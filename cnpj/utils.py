...
def calculate_dv_cnpj(cnpj_base: int) -> str:
    ...
    weight = np.array([5, 4, 3, 2, 9, 8, 7, 6, 5, 4, 3, 2])
...
def confiability_score(cnpj_base: int) -> str:
    ...
    Returns:
    str: The reliability score of the company, categorized as "Baixo" (Low), "Médio" (Medium), or "Alto" (High).
    """

    empresa_data_econo = econodata_scrapping(cnpj_base)
    empresa_data_gov = Empresas.objects.filter(cnpj_basico=cnpj_base).values()

    # Get today's date
    today = datetime.date.today()

    # Get the company date of creation
    opening_date = empresa_data_econo['data_abertura']
    opening_date = datetime.datetime.strptime(opening_date, '%d/%m/%Y').date()

    # Make opening_date subtracted by hoje in days
    degree_1 = (today - opening_date).days

    capital = empresa_data_gov[0]['capital_social']

    port = empresa_data_gov[0]['porte_empresa']

    # Get the number of employees
    no_employees = empresa_data_econo['n_funcionarios']
    no_employees = re.findall(r'\d+', no_employees)
    no_employees = max(no_employees)
    ...
    if int(no_employees) <= 10:
        degree_2 = 1
    elif int(no_employees) <= 100:
        degree_2 = 2
    elif int(no_employees) <= 1000:
        degree_2 = 3
    elif int(no_employees) <= 5000:
        degree_2 = 4
    else:
        degree_2 = 5
    ...
    score = (degree_1 + degree_2 + degree_3 + port)/4
